...
function filterProducts(searchTerm, category = '') {
    ...
    if (category) {
        filtered = filtered.filter(product => 
            (product.context?.custom?.category || 'Uncategorized') === category
        );
    }
    ...
    if (term) {
        filtered = filtered.filter(product => {
            const context = product.context?.custom || {};
...
function sortProducts(products, sortType) {
    ...
    switch (sortType) {
        case 'name-asc':
            sorted.sort((a, b) => (a.context?.custom?.name || '').localeCompare(b.context?.custom?.name || ''));
            break;
        case 'name-desc':
            sorted.sort((a, b) => (b.context?.custom?.name || '').localeCompare(a.context?.custom?.name || ''));
            break;
        case 'price-asc':
            sorted.sort((a, b) => {
                const priceA = parseFloat(a.context?.custom?.price || 0);
                const priceB = parseFloat(b.context?.custom?.price || 0);
...
function displayProducts(products, fromCache = false) {
    if (products.length === 0) {
        gallery.innerHTML = '';
        noResultsElement.style.display = 'block';
        searchInfo.textContent = 'No products found';
        updateStatistics([]);
        return;
    }
    ...
    if (searchInput.value.trim() || categoryFilter.value) {
        ...
        if (category) info += ` in ${category}`;
        
        searchInfo.textContent = info;
    } else {
        searchInfo.textContent = `Showing ${products.length} featured products`;
    }
...
async function loadProducts(forceRefresh = false) {
    ...
    try {
        ...
        console.log(`✅ Loaded ${allProducts.length} products from Hugging Face`);
...
function shareProduct(name, url) {
    if (navigator.share) {
        navigator.share({
            title: name,
            text: `Check out ${name} from CraftyUG`,
            url: url
        });
    } else {
